...
const PropertyList = () => {
  ...
  const propertiesFromStore = useSelector((state) => state.properties || {});
  const { properties = [], totalProperties = 0 } = propertiesFromStore;
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      dispatch(propertyAction.updateSearchParams({ page: currentPage.page }));
  ...
  useEffect(() => {
    if (propertyListRef.current && propertyListRef.current.children) {
      gsap.fromTo(
