...
class APIFeatures {
  ...
  search() {
    ...
    if (queryObj.city && String(queryObj.city).trim()) {
      const city = String(queryObj.city).trim();
      const withoutSpaces = city.replace(/\s+/g, "");
      const escapeRegex = (value) =>
        value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const cityPatterns = [
        new RegExp(`^${escapeRegex(city)}$`, "i"),
        new RegExp(`^${escapeRegex(withoutSpaces)}$`, "i"),
      ];

      searchQuery.$or = [
        { "address.city": { $in: cityPatterns } },
        { "address.state": { $in: cityPatterns } },
        { "address.area": { $in: cityPatterns } },
      ];
    }
